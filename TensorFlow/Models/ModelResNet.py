...
class ResNet50(Model):
    ...
    def build(cls, input_var):
        ...
        net.update(sub_net)
        net['pool1'] = Pool2DLayer(net[parent_layer_name], pool_size=3, stride=2, pad=0, mode='max', ignore_border=False)
        block_size = list('abc')
        ...
        for c in block_size:
            ...
            net.update(sub_net)
        net['pool5'] = Pool2DLayer(net[parent_layer_name], pool_size=7, stride=1, pad=0,
    ...
    def load_model_parameters(cls, filename, network):
        ...
        lasagne.layers.set_all_param_values(network, saved_parameters['values'])
        return network
